...
bool VernamCipher(char*filename1,char*filename2,char*key){//шифр Вернама, функция принимает три аргумента -
    ...
    char byte;//очередной считываемый байт из исходного текста
    char keybyte;//очередной считываемый байт из ключа
    while(file.read(&byte,sizeof(char))){
        keyfile.read(&keybyte, sizeof(char));
        char resbyte = keybyte^byte;//результирующий байт равен сумме по модулю 2 байта ключа и текста
        cipherfile.write(&resbyte, sizeof(char));//записываем результирующий байт в шифротекст
    }
    file.close();
    cipherfile.close();
    keyfile.close();
...
void generateKey(char*filename, long long size){//генерируем рандомный ключ заданного размера и записываем в файл с названием filename
    ...
    while(size>0){
        char byte = rand()%256;//генерируем случайный байт (число от 0 до 255)
